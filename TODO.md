...
- [ ] Consolidar el proyecto desde el ~35% de avance actual hacia una version experimental reproducible.
- [ ] Mantener el foco primero en cerrar el baseline y despues en ampliar la comparacion.

## Fase 1: Cerrar baseline GARCH

- [ ] Dejar `GARCHModel` como baseline formal y estable para el experimento.
- [ ] Verificar que el flujo `load -> preprocess -> fit -> predict -> evaluate` funcione sin ajustes manuales.
- [ ] Confirmar que `PredictionResult` y `EvaluationResult` sean los artefactos canonicos del pipeline.
...
- [ ] Estandarizar nombres de modelos en configuracion segun las keys reales del registry.
...
- [ ] Consolidar `Evaluator` y `ModelComparator` como capa oficial de evaluacion.
- [ ] Calcular RMSE y MAE de forma uniforme para todos los modelos.
